...
::GenerateData()
{
  ...
  OffsetVec LineOffsets(ActiveIndexes.size());
  ...
  // visits to the output image which should massively improve cache
...
void
ConnectedComponentImageFilter3< TInputImage, TOutputImage >
::CompareLines(const lineEncoding &current, const lineEncoding &Neighbour,
	       EquivalencyTable::Pointer &eqTable)
{
  ...
  typename lineEncoding::const_iterator cIt, nIt, mIt;
  ...
  for (cIt = current.begin();cIt != current.end();++cIt)
	...
	if ((ss1 <= cLast) && (ee2 >= cLast))
      ...
      if (eq) 
	{
	//if (eqTable->RecursiveLookup(cIt->label) != eqTable->RecursiveLookup(nIt->label))
